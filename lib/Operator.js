...
class Operator {
  ...
  createPoolResource() {
    if (this._pool.room > 0) {
      ...
      return this._pool.addResourceToAll(creationPlaceHolder).then(() => {
        Utils.emitMessage(EventType.DEBUG, `Before resource creation, pool size is ${this._pool.poolSize}, ` +
          `placeholder number is ${this._pool.placeholderNum}, available resource number is ${this._pool.availableResourceNum}`);
      ...
      Utils.emitMessage(EventType.DEBUG, `Pool is full, no room left.`);
  ...
  destroy(resource) {
    if (!resource) {
      Utils.emitMessage(EventType.DEBUG, `The resource is null, do not need to be destroyed.`);
      return Promise.resolve();
    }
    Utils.emitMessage(EventType.DEBUG,
      `Before destroy, pool size is ${this._pool.poolSize}, id of the resource to be destroyed is ${resource.id}`);
    ...
    Utils.emitMessage(EventType.DEBUG, `After destroy, pool size is ${this._pool.poolSize}`);
  ...
  deliverPooledConnection(resource) {
    ...
    return Factory.validate(resource.connection).then((validateResult) => {
      if (validateResult) {
        ...
        Utils.emitMessage(EventType.DEBUG, `Connection validation failed, destroying it. Resource id is ${resource.id}`);
