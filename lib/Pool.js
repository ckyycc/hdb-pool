...
class Pool {
   ...
   * @param {any} opts options of the pool
   */
  constructor(params, opts = null) {
     ...
     * @private
     */
    this._parameters = Utils.getPoolParams(params);
    /**
  ...
  addResourceToAvailable(resource) {
    ...
    if (this._options && this._options.checkInterval > 0 && this._intervalId === undefined) {
      this._intervalId = this._checkConnectionIdleTimeout(this._options.checkInterval);
  ...
  /**
   * Pooled connection idle timeout check
   * @param {number} time checkInterval
   * @return {number}
   * @private
   */
  _checkConnectionIdleTimeout(time) {
    return setInterval(() => this._notifyAllOperators(new Task(TaskType.CHECK_IDLE_TIMEOUT)).catch((err) => {
  ...
  _notifyAllOperators(task) {
    ...
    return Promise.reject(
      new Error(`Something wrong, can not find any worker for task ${task ? task.taskType ? task.taskType : 'None' : 'None'}`));
  }
}
