...
class RequestOperator extends Operator {
   ...
   */
  work(task) {
    Util.emitMessage(EventType.DEBUG, `Trying to dispatch pooled connections, task type: ${task.taskType}`);
      ...
      if (request) {
        ...
        if (this.pool.options.maxWaitingRequests > 0 && this.pool.requestList.length > this.pool.options.maxWaitingRequests) {
          Util.emitMessage(EventType.DEBUG, `Max waiting requests count exceeded, removing the request from the list, ` +
            `request list size is ${this.pool.requestList.length}`);
          this.pool.removeRequestFromList(request);
          Util.emitMessage(EventType.DEBUG, `Request list size is ${this.pool.requestList.length}`);
  ...
  _dispatchPooledConnections() {
    ...
    Util.emitMessage(EventType.DEBUG, `Dispatching pooled connections, waiting requests number is ${waitingNum}`);

    if ((waitingNum < 1) || (this.pool.availableResourceNum < 1 && this.pool.room < 1)) {
      Util.emitMessage(EventType.DEBUG, `waiting number < 1 or pool empty and no room left.`);
      return Promise.resolve();
    }
    ...
    Util.emitMessage(EventType.DEBUG, `Resource number to create: ${resourcesNumToCreate}, available resource number: ` +
      `${this.pool.availableResourceNum}, waiting request number: ${waitingNum}, placeholder number: ${this.pool.placeholderNum}`);
