...
class PoolManager {
  ...
  getPoolStatusOverView() {
    return `Pool Size: ${this._pool.poolSize} (${this._pool.options.min} : ${this._pool.options.max}),` +
      `available connection number: ${this._pool.availableResourceNum},` +
      `request number: ${this._pool.requestList.length} ` +
